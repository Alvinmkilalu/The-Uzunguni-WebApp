...
type BillItem = { id:string; name:string; quantity:number; unit_price_amount:number; total_amount:number; unavailable?:boolean };
type Bill = { table_number:number; table_label:string; session_id:string|null; status:string; items:BillItem[]; total_amount:number; paid_amount:number };
type Step = "bill"|"choice"|"split"|"items"|"amount"|"provider"|"phone"|"waiting"|"success";
...
const money=(value:number)=>`TZS ${Math.max(0,value).toLocaleString("en-US")}`;

export default function CustomerBill(){
  const[bill,setBill]=useState<Bill|null>(null),[loading,setLoading]=useState(true),[step,setStep]=useState<Step>("bill");
  ...
  const remaining=Math.max(0,(bill?.total_amount??0)-(bill?.paid_amount??0));
  const selectedTotal=useMemo(()=>(bill?.items??[]).filter(i=>selectedItems.includes(i.id)).reduce((sum,i)=>sum+i.total_amount,0),[bill,selectedItems]);
  const equalShare=Math.min(remaining,Math.ceil(remaining/people));
  const amount=splitType==="FULL"?remaining:splitType==="EQUAL"?equalShare:splitType==="ITEM"?selectedTotal:Number(customAmount||0);
  const selectedProvider=providers.find(p=>p.id===method)??providers[0];
  const go=(next:Step)=>{setError("");setStep(next)};
  function toggleItem(id:string){setSelectedItems(current=>current.includes(id)?current.filter(x=>x!==id):[...current,id])}
  ...
  if(!bill.session_id)return <main className={styles.phone}><Top table={bill.table_label}/><Empty title="No active bill" text="Ask your waiter to initiate the table after taking your order."/></main>;

  return <main className={styles.phone}><Top table={bill.table_label}/>
    {step==="bill"&&<Screen eyebrow="SECURE LIVE BILL" title="Your bill"><div className={styles.itemsCard}><p className={styles.cardLabel}>ITEMS · {bill.items.length}</p>{bill.items.map(item=><div className={styles.billItem} key={item.id}><span><b>{item.name}</b><small>Qty {item.quantity} · {money(item.unit_price_amount)} each</small></span><strong>{money(item.total_amount)}</strong></div>)}</div><div className={styles.totals}><div><span>Total</span><b>{money(bill.total_amount)}</b></div><div><span>Already paid</span><b>{money(bill.paid_amount)}</b></div><div className={styles.remaining}><span>REMAINING BALANCE</span><strong>{money(remaining)}</strong></div></div><p className={styles.live}>◷ Updated automatically</p>{remaining>0?<StickyButton onClick={()=>go("choice")}>▣ Pay {money(remaining)}</StickyButton>:<div className={styles.paidTag}>✓ Paid in full · Server verified</div>}</Screen>}

    {step==="choice"&&<Screen back={()=>go("bill")} eyebrow={`${bill.table_label.toUpperCase()} · REMAINING ${money(remaining)}`} title="How would you like to pay?"><Choice title="Full bill" text={`Pay the remaining ${money(remaining)} yourself`} icon="▣" onClick={()=>{setSplitType("FULL");go("provider")}}/><Choice title="Split bill" text="Share the bill with others at the table" icon="♧" onClick={()=>go("split")}/><p className={styles.help}>Nothing is final until you approve the secure prompt on your phone.</p></Screen>}

    {step==="split"&&<Screen back={()=>go("choice")} eyebrow={`${bill.table_label.toUpperCase()} · SPLIT BILL`} title="Split the bill"><Choice title="Split equally" text="Divide evenly by number of people" icon="♧" selected={splitType==="EQUAL"} onClick={()=>setSplitType("EQUAL")}>{splitType==="EQUAL"&&<div className={styles.stepper} onClick={e=>e.stopPropagation()}><span>Number of people</span><button onClick={()=>setPeople(Math.max(2,people-1))}>−</button><b>{people}</b><button onClick={()=>setPeople(Math.min(20,people+1))}>+</button><small>You pay {money(equalShare)} as one share</small></div>}</Choice><Choice title="Select items" text="Choose exactly what you ordered" icon="▤" onClick={()=>{setSplitType("ITEM");go("items")}}/><Choice title="Custom amount" text="Pick items to pay for, or type any amount" icon="＋" onClick={()=>{setSplitType("CUSTOM");go("amount")}}/>{splitType==="EQUAL"&&<StickyButton onClick={()=>go("provider")}>Continue · {money(equalShare)}</StickyButton>}</Screen>}

    {step==="items"&&<Screen back={()=>go("split")} eyebrow={`${bill.table_label.toUpperCase()} · SELECT ITEMS`} title="Select items to pay"><div className={styles.itemsCard}><p className={styles.cardLabel}>TAP TO SELECT · {bill.items.length} ITEMS</p>{bill.items.map(item=><button className={`${styles.selectItem} ${selectedItems.includes(item.id)?styles.checked:""}`} key={item.id} disabled={item.unavailable} onClick={()=>toggleItem(item.id)}><span className={styles.checkbox}>{item.unavailable?"–":selectedItems.includes(item.id)?"✓":""}</span><span><b>{item.name}</b><small>{item.unavailable?"Already paid or reserved":`Qty ${item.quantity} · ${money(item.unit_price_amount)} each`}</small></span><strong>{money(item.total_amount)}</strong></button>)}</div><p className={styles.help}>Locked items cannot be selected again while another payment is pending or completed.</p><StickyButton disabled={!selectedTotal} onClick={()=>go("provider")}>Pay {money(selectedTotal)}</StickyButton></Screen>}

    {step==="amount"&&<Screen back={()=>go("split")} eyebrow={`${bill.table_label.toUpperCase()} · CUSTOM AMOUNT`} title="Pay a custom amount"><div className={styles.tabs}><button className={styles.activeTab}>Type amount</button><button onClick={()=>{setSplitType("ITEM");go("items")}}>Pick items</button></div><p className={styles.amountHint}>OF {money(remaining)} REMAINING</p><label className={styles.amountInput}><span>TZS</span><input value={customAmount} onChange={e=>setCustomAmount(e.target.value.replace(/\D/g,""))} inputMode="numeric" placeholder="0" autoFocus/></label><div className={styles.quickAmounts}>{[25,50,75].map(percent=><button key={percent} onClick={()=>setCustomAmount(String(Math.floor(remaining*percent/100)))}>{percent}%</button>)}<button onClick={()=>setCustomAmount(String(remaining))}>Remaining</button></div>{error&&<p className={styles.error}>⚠ {error}</p>}<StickyButton disabled={!Number(customAmount)||Number(customAmount)>remaining} onClick={()=>go("provider")}>Pay {money(Number(customAmount||0))}</StickyButton></Screen>}

    {step==="provider"&&<Screen back={()=>go(splitType==="FULL"?"choice":splitType==="EQUAL"?"split":splitType==="ITEM"?"items":"amount")} eyebrow={`${splitType.replace("_"," ")} · ${money(amount)}`} title="Choose how to pay"><p className={styles.cardLabel}>MOBILE MONEY</p><div className={styles.providers}>{providers.map(provider=><button key={provider.id} className={method===provider.id?styles.providerSelected:""} onClick={()=>setMethod(provider.id)}><span className={styles.providerIcon}>{provider.initials}</span><span><b>{provider.name}</b><small>Push prompt to your phone</small></span><strong>›</strong></button>)}</div><StickyButton onClick={()=>go("phone")}>Continue with {selectedProvider.name}</StickyButton></Screen>}
...
function Empty({title,text}:{title:string;text:string}){return <div className={styles.center}><div className={styles.successIcon}>✓</div><h1>{title}</h1><p>{text}</p></div>}
