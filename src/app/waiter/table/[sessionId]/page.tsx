...
export default async function WorkspacePage({params}:{params:Promise<{sessionId:string}>}) {
  ...
  const {data:session}=await supabase.from("table_sessions").select("id,status,opened_at,dining_tables(table_number,label)").eq("id",sessionId).single(); if(!session)notFound();
